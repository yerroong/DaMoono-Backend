...
- 
    ...
    **예시 - 제외:**
    ...
    추출 기준 (하나라도 해당되면 제시안으로 분류):
    
    1. 상담사가 "방법이 두 가지 있습니다", "A 또는 B 중 선택하세요", "어떤 것으로 진행할까요?", “선택지가 있습니다”와 같이 명시적으로 선택지를 제시한 경우.
    2. 고객의 결정(Decision)에 의해 최종 처리 결과가 달라지는 경우.
    3. 명확한 비교 수치(가격, 용량, 기간 등)가 포함된 대안들.
    
    💡 추출 팁:
    
    - 결과적으로 고객에게 "선택"을 맡기고 그에 따라 처리가 이뤄졌다면 proposals로 분류하십시오.
    - 단순 안내가 아닌 '의사결정의 대상'이 된 아이템들을 리스트업 하십시오.
